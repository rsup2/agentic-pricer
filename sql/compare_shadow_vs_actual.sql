...
WITH shadow AS (
    ...
    SELECT
        ...
        ANY_VALUE(PRICER_LABEL)                       AS pricer_label,   -- which build produced this
        ANY_VALUE(PRICER_VERSION)                     AS pricer_version, -- exact commit SHA
...
-- (pricer_label) x confidence — so you compare version-over-version and never pool
-- prices from two code cuts. Add `pricer_version` to see the exact commit, or filter
-- `WHERE pricer_label = '<build>'` to score one build in isolation.
SELECT
    pricer_label,
...
FROM joined
GROUP BY pricer_label, shadow_conf_rank
ORDER BY pricer_label, shadow_conf_rank DESC;
...
FROM joined
GROUP BY pricer_label, segment
ORDER BY pricer_label, segment;
